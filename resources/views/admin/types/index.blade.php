...
<div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
    <div class="p-6 border-b border-gray-100 flex justify-between items-center bg-gray-50/50">
        <div>
            <h2 class="text-xl font-bold text-gray-800">Daftar Tipe Produk</h2>
            <p class="text-sm text-gray-500 mt-1">Kelola daftar tipe kosmetik (seperti Lip Cream, Lip Tint, dll).</p>
        </div>
        <a href="{{ route('admin.types.create') }}" class="inline-flex items-center justify-center px-4 py-2 bg-rose-500 text-white font-medium text-sm rounded-lg hover:bg-rose-600 transition-colors shadow-sm">
        ...
        <table class="w-full text-left text-sm text-gray-600">
            <thead class="bg-gray-50 text-gray-700 font-medium border-b border-gray-200">
                <tr>
                    ...
                    <th class="px-6 py-4">Nama Tipe Produk</th>
            ...
            <tbody class="divide-y divide-gray-100">
                ...
                <tr class="hover:bg-gray-50/50 transition-colors">
                    <td class="px-6 py-4 text-gray-500">{{ $type->id }}</td>
                    <td class="px-6 py-4 font-medium text-gray-900">{{ $type->name }}</td>
                    <td class="px-6 py-4 text-center">
                        <div class="flex items-center justify-center space-x-3">
                            <a href="{{ route('admin.types.edit', $type->id) }}" class="text-amber-500 hover:text-amber-600 transition-colors" title="Edit">
                                <svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5">
                                    <path stroke-linecap="round" stroke-linejoin="round" d="M16.862 4.487l1.687-1.688a1.875 1.875 0 112.652 2.652L10.582 16.07a4.5 4.5 0 01-1.897 1.13L6 18l.8-2.685a4.5 4.5 0 011.13-1.897l8.932-8.931zm0 0L19.5 7.125M18 14v4.75A2.25 2.25 0 0115.75 21H5.25A2.25 2.25 0 013 18.75V8.25A2.25 2.25 0 015.25 6H10" />
                                </svg>
                            </a>
                            <form action="{{ route('admin.types.destroy', $type->id) }}" method="POST" class="inline-block" onsubmit="return confirm('Apakah Anda yakin ingin menghapus tipe produk ini?');">
                                @csrf
                                @method('DELETE')
